...
### デモアカウント（要作成）
Apple 審査員がメール/パスワードでログインできる専用アカウントを用意する（Sign in with Apple/Google も利用可だが、審査員はメール/パスワードを使うのが一般的）。

- 提案: **`appreview@noshi.me` / パスワードは別途生成**（16文字程度）。
- 種データ: 贈答記録を数件＋お返し提案が見えるよう seed すると審査がスムーズ。
- ※ 作成は AWS（Cognito ユーザー作成＋DynamoDB seed）で実施可能。実行する場合は別途。
