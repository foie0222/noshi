...
> [!IMPORTANT]
> **2026-09 に実現方法が変わりました（Issue #458）。** 続柄グループでの出し分けという
> 考え方と `backend/app/catalog/relationships.py`（`group_of`）は現行も使われていますが、
> 土台だった楽天カタログは撤去され、並べ替えは LLM の fit スコアではなく
> `backend/app/catalog/guide.py` の静的な相性指定になりました。グループ別 CTR の計測も
> クリック計測 API ごと廃止されています。以下の楽天 API・バッチ・計測に関する記述は
> 当時の記録であり、現行仕様ではありません。
