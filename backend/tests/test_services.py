...
def test_returns_payloadはcategory素通しと品目タブを1回の認可で返す():
    ...
    payload = svc.returns_payload("u1", ev.id, 5000, "友人", "出産祝い", category="towel")
...
def test_お返しをgivenレコードで記録しreturn_for_idで紐付けられる():
    """received 記録のお返しを given レコードとして保存し、return_for_id で紐付けられることを検証する。"""
    svc = make_service()
    received, _ = svc.create_record(
        "u1", amount=30000, purpose="出産祝い", party_name="佐藤", direction="received"
    )
    given, ev = svc.create_record(
        "u1",
        amount=15000,
        purpose="出産祝い",
        party_name="佐藤",
        direction="given",
        return_for_id=received.id,
    )
    assert given.return_for_id == received.id
    assert ev is None  # given はイベントを生成しない


def test_別世帯のreceivedへのreturn_for_idは拒否される():
    """別世帯のレコードIDを return_for_id に指定しても ValidationError で拒否されることを検証する（A01）。"""
    svc = make_service()
    received_u1, _ = svc.create_record(
        "u1", amount=20000, purpose="香典", party_name="田中", direction="received"
    )
    with pytest.raises(ValidationError):
        svc.create_record(
            "u2",
            ...
            party_name="田中",
            direction="given",
            return_for_id=received_u1.id,  # 別世帯のレコードIDは存在しないとして拒否
        )


def test_givenレコードへのreturn_for_id紐付けは拒否される():
    """direction=given のレコードを return_for_id に指定した場合は ValidationError になることを検証する。"""
    svc = make_service()
    given_rec, _ = svc.create_record(
        "u1", amount=10000, purpose="お歳暮", party_name="山田", direction="given"
    )
    with pytest.raises(ValidationError):
        svc.create_record(
            "u1",
