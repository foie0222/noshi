...
def test_贈与税集計の除外用途は仕様の3用途のみ():
    """御霊前・御仏前は仕様BR-4-TAX-1の列挙に無いため集計から除外しないことを検証する。"""
    recs = [
        _rec(200000, "御霊前"),  # 仕様外: 除外しない → 集計に含める
        _rec(100000, "御仏前"),  # 仕様外: 除外しない → 集計に含める
        _rec(50000, "香典"),  # 仕様内: 除外する
    ]
    s = rules.gift_tax_summary(recs, year=2026)
    assert s["total"] == 300000  # 御霊前 + 御仏前 のみ集計、香典は除外
